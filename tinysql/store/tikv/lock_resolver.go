...
func (lr *LockResolver) getTxnStatus(bo *Backoffer, txnID uint64, primary []byte, callerStartTS, currentTS uint64, rollbackIfNotExist bool) (TxnStatus, error) {
	...
	for {
		...
		// YOUR CODE HERE (lab2).
		status = TxnStatus{ttl: cmdResp.LockTtl, commitTS: cmdResp.CommitVersion, action: cmdResp.Action}
		return status, nil
	}
}
...
func (lr *LockResolver) resolveLock(bo *Backoffer, l *Lock, status TxnStatus, cleanRegions map[RegionVerID]struct{}) error {
	...
	for {
		...
		// build the request
		// YOUR CODE HERE (lab2).
		request := &kvrpcpb.ResolveLockRequest{Context: &kvrpcpb.Context{}, StartVersion: l.TxnID, CommitVersion: status.commitTS} // TODO：这里的StartVersion不确定
		req = tikvrpc.NewRequest(tikvrpc.CmdResolveLock, request, kvrpcpb.Context{})
