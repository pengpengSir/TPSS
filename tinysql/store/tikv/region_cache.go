...
type Region struct {
	...
	store      unsafe.Pointer // point to region store info, see RegionStore,类似于C语言中的void*,指向 RegionStore
...
//
// Help function `RegionCache.LocateKey`: LocateKey searches for the region and range that the key is located.
// RegionCache caches Regions loaded from PD.
func (c *RegionCache) GroupKeysByRegion(bo *Backoffer, keys [][]byte, filter func(key, regionStartKey []byte) bool) (map[RegionVerID][][]byte, RegionVerID, error) {
	// YOUR CODE HERE (lab2).
	// 注意2pc.go 259行调入函数时传入的filter是nil
	resMap := make(map[RegionVerID][][]byte)

	// 先找到pk
	pk := keys[0]
	keyLocation, err := c.LocateKey(bo, pk)
	pkRegionVerID := keyLocation.Region
	resMap[pkRegionVerID] = append(resMap[pkRegionVerID], pk)
	if err != nil {
		return nil, RegionVerID{0, 0, 0}, err
	}

	// 处理剩下的secondary keys
	for i := 1; i < len(keys); i++ {
		secondKey := keys[i]
		secondKeyLocation, err := c.LocateKey(bo, secondKey)
		if err != nil {
			return nil, RegionVerID{}, nil
		}

		secondRegionVerID := secondKeyLocation.Region
