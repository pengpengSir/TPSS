...
- 运行部分实验需要约 4GB 内存，如果运行测试时遇到内存不足的相关错误，可以尝试调低 `GOGC` 环境变量，详见*本地编码和测试*一节
- 硬盘推荐使用固态硬盘，预留至少 50GB 空间

## 实验准备

本 lab 的所有实验相关组件均使用 [`golang`](https://go.dev/) 实现，需要在开发机配置 golang 相关开发环境。
...
[`go module`](https://go.dev/ref/mod) 为 golang 推荐的依赖包管理方式，需要注意的是使用 go mod 方式需要确保环境变量 `GO111MODULE` 处于 `ON` 状态。本 lab 所有实验相关组件
...
参考 [classroom doc](./docs/classroom.md) 进入 github classroom 之后，将会自动生成 `distributed-txn` 对应个人 private repo，尝试将该 repo clone 到本地
```
git clone git@github.com/tiny-talent/distributed-txn-yourid.git .
```
...
注意做完全部 2 个 lab 之后才会得到全部的分数并显示通过。
