...
func (s *StandAloneStorage) Reader(ctx *kvrpcpb.Context) (storage.StorageReader, error) {
	...
	//defer txn.Discard(),之前一直不同过test的原因是返回Reader之后就结束了这个txn，实际上得取值结束之后才能结束txn，不要显式使用txn.Discard()
	BadgerReader := NewBadgerReader(txn)
...
func (s *StandAloneStorage) Write(ctx *kvrpcpb.Context, batch []storage.Modify) error {
	...
	for i := 0; i < len(batch); i++ {
		...
		cf := batch[i].Cf()
		value := batch[i].Value()
		engine_util.PutCF(s.db, cf, key, value)
	}
	return nil
}

type BadgerReader struct { // BadgerReader 实现了StorageReader的所有方法，自动继承了该接口
	txn *badger.Txn
}
