...
func TestRollbackOtherTxn4C(t *testing.T) {
	builder := newBuilder(t)
	cmd := builder.rollbackRequest([]byte{3}) //这里面的startVersion = 100

	builder.init([]kv{
		{cf: engine_util.CfDefault, key: []byte{3}, ts: 80, value: []byte{42}},
		{cf: engine_util.CfLock, key: []byte{3}, value: []byte{1, 1, 0, 0, 0, 0, 0, 0, 0, 80, 0, 0, 0, 0, 0, 0, 0, 0}}, // value组成：PK(1B), kind(1B), lock.Ts(8B 大端), lock.Ttl(8B 大端)
	})
	resp := builder.runOneRequest(cmd).(*kvrpcpb.BatchRollbackResponse)
...
func TestCheckTxnStatusTtlExpired4C(t *testing.T) {
	builder := newBuilder(t)
	cmd := builder.checkTxnStatusRequest([]byte{3}) // pk是asc码为3的
	builder.init([]kv{
		{cf: engine_util.CfDefault, key: []byte{3}, ts: cmd.LockTs, value: []byte{42}},
		{cf: engine_util.CfLock, key: []byte{3}, value: []byte{3, 1, 0, 0, 5, 0, 0, 0, 0, builder.ts(), 0, 0, 0, 0, 0, 0, 0, 8}}, // lock.Ttl = 8
	})
