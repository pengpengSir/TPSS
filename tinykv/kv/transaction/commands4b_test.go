...
func TestGetLocked4B(t *testing.T) {
	...
	// 有一个正在写的kv和lock
	builder.init([]kv{
		{cf: engine_util.CfDefault, key: []byte{99}, ts: 50, value: []byte{42}},                       // key: c, value: *
		{cf: engine_util.CfWrite, key: []byte{99}, ts: 54, value: []byte{1, 0, 0, 0, 0, 0, 0, 0, 50}}, // value中50代表2，表示Cfwrite，50前面的是write.ToBytes()
		// value是lock.ToBytes()，第一个字节是PK，第二个字节Kind， 8个字节Ts：200， 8个字节TTL
