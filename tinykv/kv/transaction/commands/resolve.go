...
func (rl *ResolveLock) PrepareWrites(txn *mvcc.MvccTxn) (interface{}, error) {
	...
	for _, kl := range rl.keyLocks {
		...
		if commitTs == 0 && rl.request.StartVersion == kl.Lock.Ts {
			txn.PutWrite(kl.Key, rl.startTs, &(mvcc.Write{rl.StartTs(), mvcc.WriteKindRollback}))
			txn.DeleteValue(kl.Key)
			txn.DeleteLock(kl.Key)
		}

		if commitTs > 0 && rl.request.StartVersion == kl.Lock.Ts {
			txn.PutWrite(kl.Key, commitTs, &(mvcc.Write{rl.StartTs(), mvcc.WriteKindPut}))
			txn.DeleteLock(kl.Key)
		}
